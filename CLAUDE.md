...
服务编排：`shortlink-mysql`（MySQL 8.0）+ `shortlink-redis`（Redis 7）+ `shortlink-app`（Spring Boot）

Docker 环境下使用 `application-docker.properties`（spring.profiles.active=docker），MySQL/Redis 地址通过容器名连接。

## API 文档

访问 `http://localhost:8080/swagger-ui.html` 在线查看和调试所有接口（SpringDoc OpenAPI 自动生成）。
